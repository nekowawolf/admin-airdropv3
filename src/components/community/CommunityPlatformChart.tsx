...
export default function CommunityPlatformChart({ data }: { data: any[] }) {
  const platformCount = data.reduce((acc: any, curr: any) => {
    if (typeof curr.platforms === 'string') {
      const platformsList = curr.platforms.split(',').map((p: string) => p.trim()).filter(Boolean)
  ...
  if (!data || data.length === 0 || Object.keys(platformCount).length === 0) {
    return (
  ...
  return (
    <div className="p-4 rounded-2xl shadow-md bg-[var(--fill-color)] border border-border-divider h-80">
      <h3 className="text-lg font-semibold text-primary mb-4">Communities by Platform</h3>
